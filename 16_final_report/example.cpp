...
void matmult(vector<float> &A, vector<float> &B, vector<float> &C, int N, int size, int offset) {
  //B_k*n, A_m*k, C_m*n
  //Bc_kc*nc, Ac_mc*kc, Cc_mc*nc
  const int m = N/size, n = N/size, k = N;
  const int kc = 512;//Bc行
  const int nc = 64;//Bc列
  const int mc = 256;//Ac行
  const int nr = 64;//Bc列再び分割
  const int mr = 32;//Ac行再び分割
#pragma omp parallel for collapse(2)
  for (int jc=0; jc<n; jc+=nc) {
    for (int pc=0; pc<k; pc+=kc) {
      ...
      for (int p=0; p<kc; p++) {
        for (int j=0; j<nc; j++) {
          Bc[p*nc+j] = B[(p+pc)*n+(j+jc)];//B[p+pc][j+jc]
        }
      }
  ...
  float Ac[mc*kc],Cc[mc*nc];
        for (int i=0; i<mc; i++) {
          for (int p=0; p<kc; p++) {
            Ac[i*kc+p] = A[(i+ic)*k+p+pc];//A[i+ic][p+pc]
          }
          for (int j=0; j<nc; j++) {
            Cc[i*nc+j] = 0;
          }
